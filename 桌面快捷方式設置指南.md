...
```bash
# 在新的終端中執行
~/wakeupmap-pi/raspberrypi-dsi/stop-wakeup-map.sh
```
...
Exec=/home/yutingpi/wakeupmap-pi/raspberrypi-dsi/start-wakeup-map.sh
Icon=/home/yutingpi/wakeupmap-pi/icon-192x192.png
Terminal=true
StartupNotify=true
Categories=Utility;
...
```bash
# 如果你有自己的圖標文件，可以替換
cp 你的圖標.png ~/wakeupmap-pi/icon-192x192.png
...
📁 工作目錄: /home/yutingpi/wakeupmap-pi/raspberrypi-dsi
📝 日誌文件: /home/yutingpi/wakeupmap-pi/wakeup-map.log

🚀 使用主控制程式: main_web_dsi.py
...
```bash
# 創建啟動腳本
nano ~/wakeupmap-pi/raspberrypi-dsi/start-wakeup-map.sh
```

貼上啟動腳本內容，然後：
```bash
# 設置執行權限
chmod +x ~/wakeupmap-pi/raspberrypi-dsi/start-wakeup-map.sh
```
...
```bash
# 直接執行啟動腳本
~/wakeupmap-pi/raspberrypi-dsi/start-wakeup-map.sh
```

### **檢查進程狀態**
```bash
# 檢查是否有甦醒地圖進程在運行
pgrep -f "main_web_dsi.py"

# 查看詳細進程信息
ps aux | grep "main_web_dsi.py"
```
...
```bash
# 檢查路徑是否正確
ls -la ~/wakeupmap-pi/raspberrypi-dsi/
ls -la ~/wakeupmap-pi/raspberrypi-dsi/main_web_dsi.py
ls -la ~/wakeupmap-pi/raspberrypi-dsi/main_web_dsi.py
```
...
chmod +x ~/Desktop/甦醒地圖.desktop
chmod +x ~/wakeupmap-pi/raspberrypi-dsi/start-wakeup-map.sh
chmod +x ~/wakeupmap-pi/raspberrypi-dsi/stop-wakeup-map.sh
```
